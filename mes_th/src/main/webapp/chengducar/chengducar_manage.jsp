...
<html>
	...
	<%   
    final  Log log = LogFactory.getLog("chengducar_manage.jsp");
	response.setHeader("progma","no-cache");
	response.setHeader("Cache-Control","no-cache");
    Connection con = null;
    Statement stmt = null;
    ResultSet rs = null;  
    List<com.qm.th.tg.bean.Part> list_part = new ArrayList<com.qm.th.tg.bean.Part>();//总成集合
	...
	String today = "";
	String startDate = "";
	SimpleDateFormat sdf_chengdu = new SimpleDateFormat("yyyy-MM-dd");
	try{
	    //获取连接
	    con=Conn.getConn();
		startDate = request.getParameter("startDate");
		today = sdf_chengdu.format(new Date());
		log.debug(today);
		log.debug(startDate);
	    ...
	    switch(radio_value){
			...
			list_part = factory_ws.getPartsByStartTimeEndTime_chengdu(condition_One,condition_Two,ss.getCcarType(),con);
			sql_part = dao.getPartsByStartTimeEndTime_chengdu(condition_One,condition_Two,ss.getCcarType());
			list_ws = factory_ws.getStatByStartTimeEndTime_chengdu(condition_One,condition_Two,ss.getCcarType(),con);
			sql_ws = dao.getStatByStartTimeEndTime_chengdu(condition_One,condition_Two,ss.getCcarType());
			...
			list_part = factory_ws.getPartsByStartOrderEndOrder_chengdu(condition_One,condition_Two,ss.getCcarType(),con);
			sql_part = dao.getPartsByStartOrderEndOrder_chengdu(condition_One,condition_Two,ss.getCcarType());
			list_ws = factory_ws.getStatByStartOrderEndOrder_chengdu(condition_One,condition_Two,ss.getCcarType(),con);
			sql_ws = dao.getStatByStartOrderEndOrder_chengdu(condition_One,condition_Two,ss.getCcarType());
			...
			list_part = factory_ws.getPartsByStartTimeNum_chengdu(condition_One,condition_Two,ss.getCcarType(),con);
			sql_part = dao.getPartsByStartTimeNum_chengdu(condition_One,condition_Two,ss.getCcarType());
			list_ws = factory_ws.getStatByStartTimeNum_chengdu(condition_One,condition_Two,ss.getCcarType(),con);
			sql_ws = dao.getStatByStartTimeNum_chengdu(condition_One,condition_Two,ss.getCcarType());
			sql_temp3 = "where cx.dWBegin>=convert(varchar(100),'" + condition_One + "',20) ";
	    ...
	    ||radio_value==3&&request.getParameter("d13").equals("")&&request.getParameter("num").equals("")){
	    	...
	    	list_part = factory_ws.getParts_chengdu(ss.getCcarType(),con);

	    	sql_part = dao.getParts_chengdu(ss.getCcarType());

	    	list_ws = factory_ws.getStat_chengdu(ss.getCcarType(),con);
			sql_ws = dao.getStat_chengdu(ss.getCcarType());
	    	sql_temp3 = "where cx.dWBegin is not null and cx.dABegin is null and dCp6Begin is null "; 
		}

	    sql_temp3 = sql_temp3 + " and substring(cx.cCarNo,5,1)='7' and substring(cx.cCarNo,6,1) in(" + ss.getCcarType() +") ";
...
.head2 {
	...
	<body background="../images/background.jpg">
		...
		<div class="title">
			<strong>
				<!-- InstanceBeginEditable name="标题2" -->成都车查询<!-- InstanceEndEditable -->
			</strong>
		</div>
		<br>
		<div align="center">
			...
			<form action="chengducar_manage.jsp">
				<table>
					<tr>
						<td>
					...
					<tr>
						<td>
							...
							<input name="startOrder"
								value="<%=radio_value != 2 || condition_One.equals("0") ? ""
						: condition_One%>">
							--结束顺序号
							<input name="endOrder"
								value="<%=radio_value != 2 || condition_Two.equals("0") ? ""
						: condition_Two%>">
					...
					<tr>
						...
						: condition_One%>" />
							...
							--数量
							<input name="num"
								value="<%=radio_value != 3 || condition_Two.equals("0") ? ""
						: condition_Two%>">
						</td>
					</tr>
				</table>
...
tr {
	...
	<script type="text/javascript">
function deletecar(){
	window.location.href="chengducar_deleting.jsp?startDate="+document.getElementsByName("startDate")[0].value;
}
function check_radio(){
	document.getElementsByName("radio1")[<%=radio_value - 1%>].checked = true;
}//check_radio();
function xls_select(){
	window.location.href="../inquiries/welding_select_xls.jsp?sql=<%=sql%>&sql_part=<%=sql_part%>&downloadname=焊装查询&partnum=<%=list_part.size()%>";
}
function xls_stat(){
	window.location.href="../inquiries/welding_select_xls.jsp?sql=<%=sql_ws%>&sql_part=<%=sql_part%>&downloadname=焊装统计";
}
