...
<jsp:directive.page import="mes.beans.Service" />
	...
	<body background="../images/background.jpg">
		...
		<div align="center">
			<mes:table reSourceURL="../JarResource/" id="table1">
				...
				<mes:tbody iterator="<%=list.iterator()%>" type="mes.beans.Service"
					...
					<mes:tr id="tr${service.id}">
						...
						<td>
							<a
								href='service_look.jsp?serviceid=${service.id}&intPage=<%=intPage %>&info=<%=info%>'>查看</a>
						</td>
						<td>
							<a
								href='service_update.jsp?serviceid=${service.id}&intPage=<%=intPage %>&info=<%=info%>'>更新</a>
						</td>
						<td>
							<a href="#"
								onClick="javascript:if(confirm('真的要删除这条纪录吗?')) window.location.href='service_deleting.jsp?serviceid=${service.id}&intPage=<%=intPage %>&info=<%=info%>';return false;">删除</a>
						</td>
					</mes:tr>
				</mes:tbody>
				<mes:tfoot>
		   ...
		   out.println("页");
								...
								<%if(intPage>1){    
	    	out.println("<a href=\"service_manage.jsp?page=1&info="+info+"\">第一页</a>");	
	    	out.println("<a href=\"service_manage.jsp?page="+(intPage-1)+"&info="+info+"\">上一页</a>");		
	    	}%>
								<%if(intPage<PageCount){
		    out.println("<a href=\"service_manage.jsp?page="+(intPage+1)+"&info="+info+"\">下一页</a>");
		    out.println("<a href=\"service_manage.jsp?page="+(PageCount)+"&info="+info+"\">最后一页</a>");
		    }%>

								<a href='service_add.jsp?intPage=<%=intPage %>&info=<%=info%>'>【添加新记录】</a>
							</form>
						</td>
					</mes:tr>
				</mes:tfoot>
