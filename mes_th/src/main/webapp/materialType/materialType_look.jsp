...
<html>
 <jsp:useBean id="Conn" scope="page" class="common.Conn_MES"/><!-- InstanceEndEditable -->
<%
 	String element_name = request.getParameter("element_name");
	if(element_name==null){
		out.println("<script>alert(\"参数为空\"); window.location.href=\"materialType_view.jsp\";</script>");
		return;
	} 
	...
	Statement stmt=null;
  	try{//获取连接
		...
		IMaterialType newType=factory.queryElement(new Integer(element_name),conn);
		IMaterialType parentType = null;
		String parentType_name = "";
		if(newType.getParentId()!=-1){
	       parentType = factory.queryElement(newType.getParentId(),conn);
	       parentType_name = parentType.getName();
	    }else{
	       parentType_name = null;
	    }
		if(newType==null){
			out.println("<script>alert(\"没有查询结果！\");window.location.href='materialType_view.jsp';</script>");
		}
		%>
	<head>
    ...
    <table class="tbnoborder">
      <tr>
        <td width="180">元素号 - 物料类型名：</td>
        <td width="266"><input readonly size="50" type="text" value="<%=newType.getId()%> - <%=newType.getName()==null?"":newType.getName() %>" class="box1" onMouseOver="this.className='box3'" onFocus="this.className='box3'" onMouseOut="this.className='box1'" onBlur="this.className='box1'">
        </td>
      </tr>
       <tr>
        <td>描述信息：</td>
       ...
       <tr>
        <td>父物料类型号 - 物料类型名：</td>
        <td><input readonly size="50" type="text" value="<%=newType.getParentId() %> - <%=parentType_name==null?"":parentType_name %>" class="box1" onMouseOver="this.className='box3'" onFocus="this.className='box3'" onMouseOut="this.className='box1'" onBlur="this.className='box1'">
        </td>
      </tr>
      <tr>
        <td>版本号：</td>
