...
Ext.layout.ColumnLayout = Ext.extend(Ext.layout.ContainerLayout, {
			...
			onLayout : function(C, F) {
				...
				var J = Ext.isIE && F.dom != Ext.getBody().dom ? F
						.getStyleSize() : F.getViewSize();
...
Ext.DatePicker = Ext.extend(Ext.Component, {
	...
	update : function(a, U) {
		...
		if (this.showToday) {
			...
			var B = (I < O || I > S || (Z && T && Z.test(I.dateFormat(T))) || (c && c
					.indexOf(I.getDay()) != -1));
		...
		this.mbtn
				.setText(this.monthNames[a.getMonth()] + " " + a.getFullYear());
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
	...
	ensureVisible : function(P, E, D) {
		...
		return F ? Ext.fly(F).getXY() : [L.scrollLeft + this.el.getX(),
				Ext.fly(I).getY()]
	},
