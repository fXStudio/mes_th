...
<% 
	  ...
	  //}
		...
		String condition=" where 0=0 and outemp is null ";
        ...
        int rows=new Terminal().getCarFilterCount(condition);
	   	int total=rows;
	   	Connection con = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            String sql="select * from car_pageno "+condition;
            ...
            for (int i=index; i < aa; i++) {
                if(rs.next()) {
                    output += "{car:'" + rs.getString("car")+ "',recorddate:'" + format.format(rs.getTimestamp("recorddate")) + "',emp:'"+rs.getString("emp")+"'}";
                    if (i != aa - 1) {
                        output += ",";
                    }
                }
