...
.nscan {
			...
			var requestConfig = {
				...
				callback : function(options, success, response) {
					...
					if (intxt.readOnly) {
						...
						document.getElementById("seqtxt").focus();
					}
				}
			}
			intxt.readOnly = true;
	...
	function seqKeyUp() {
		...
		if (13 == event.keyCode) {
			var intxt = document.getElementById("inptxt");
			var seqtxt = document.getElementById('seqtxt');
			var requestConfig = {
				url : "terminalOperator.jsp",//请求的服务器地址
				params : {
					cPageNo : document.getElementById("indtxt").value
				...
				callback : function(options, success, response) {
					...
					if(parseInt(seqtxt.value) !== parseInt(index) + 1) {
						alert('序列号错误');
						seqtxt.focus();
...
document.getElementById("inptxt").focus();
			...
			if (destination.trim() == curscan.trim()) {
				var requestConfig = {
					...
					callback : function(options, success, response) {
						var jsonObj = eval( "(" + response.responseText + ")");
						var partName = jsonObj["partName"];
						var isNull = (!partName && partName == curscan.trim());
						if(partName || isNull){
							soundManager.play('success');
							if(isNull){
								document.getElementById("inptxt").focus();
			...
			sh.className = "";
			sh.value = arr[intxt["index"]];
			//loopSound(soundManager.getSoundById('failed'));
			loopSound(s);
			//soundManager.play('failed');
			alert('零件号错误');
			soundManager.stop('failed');
document.getElementById("inptxt").focus();
			intxt.value = "";
			turn();
			return false;
...
//-->
	...
	<body>
		...
		<div>
			<input type="text" id="seqtxt" onkeyup="seqKeyUp();" size="28">
		</div>
