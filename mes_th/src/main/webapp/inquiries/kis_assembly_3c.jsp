...
ResultSet rs = null;
	...
	try{
		...
		sql = "select c.cSEQNo_A,SUBSTRING(c.cVinCode,7,2) cartype,c.cVinCode,c.cCarNo," + 
			...
			"left join (select a.cQADNo,a.iCarId from carData_D a where a.ITFASSNameId=3) a on a.iCarId=c.cCarNo " +
			"left join (select b.cQADNo,b.iCarId from carData_D b where b.ITFASSNameId=4) b on b.iCarId=c.cCarNo " +
			"left join (select d.cQADNo,d.iCarId from carData_D d where d.ITFASSNameId=10) d on d.iCarId=c.cCarNo " +
...
<div class="title"><strong><!-- InstanceBeginEditable name="标题2" --><%=ss.getCdscCarType()+ss.getCdscFactory()%>总装FIS查询<!-- InstanceEndEditable --></strong></div>
  ...
  <table border="0" cellspacing="0" cellpadding="0" id="table1">
  	...
  	<%while(rs.next()){%>
		...
		<td align="center" style="background-color: red"><%=rs.getString(1)==null?"-":rs.getString(1)%></font></td>
