...
<div class="title"><strong>物料特征</strong></div>
	<br>
	<div align="center">
...
<mes:tr>   
		...
		<mes:td ><a href='materialCharacter_look.jsp?element_name=${mcharacter.id }'>查看</a></mes:td>
		<mes:td ><a href='materialCharacter_update.jsp?element_name=${mcharacter.id }'>更改</a></mes:td>
		<mes:td >
		<%
		IMaterialCharacter imc = (IMaterialCharacter)pageContext.getAttribute("mcharacter");
		if(imc.isDiscard()==false){
		 %>
		<a href="#" title="点击将删除这条记录！" onClick="javascript:if(confirm('真的要删除这条记录吗？')) window.location.href='materialCharacter_deleting.jsp?element_name=${mcharacter.id }';return false;">删除</a>
		<%}else{ %>
		--
		<%} %>
		</mes:td>
...
<mes:tfoot>
   <mes:tr>
     <mes:td colspan="100%" align="center">
		...
		<%if(intPage>1){    
		out.println("<a href=\"materialCharacter_view.jsp?page=1&type_info="+type_info+"\">第一页</a>");	
		out.println("<a href=\"materialCharacter_view.jsp?page="+(intPage-1)+"&type_info="+type_info+"\">上一页</a>");		
		}%>	 
		<%if(intPage<PageCount){
		out.println("<a href=\"materialCharacter_view.jsp?page="+(intPage+1)+"&type_info="+type_info+"\">下一页</a>");
		out.println("<a href=\"materialCharacter_view.jsp?page="+(PageCount)+"&type_info="+type_info+"\">最后一页</a>");
		}%>

       <a href='materialCharacter_insert.jsp'>【添加新记录】</a>
	  </mes:td>
   </mes:tr>  
   </mes:tfoot>  
</mes:table>
<form  name="form1" method="post" onSubmit="return checkinput(this)" action=""><!-- action跳转到本页面可以不写本页名,以防写错 -->
<table class="tbnoborder">
<tr>
<td class="tbnoborder">按物料特征名查询&nbsp;&nbsp;</td>
<td class="tdnoborder">
