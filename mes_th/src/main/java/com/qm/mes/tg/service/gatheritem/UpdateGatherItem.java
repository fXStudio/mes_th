 ...
 * 
 */
public class UpdateGatherItem extends AdapterService {
	// 获得连接
	...
	//日志
	private final Log log = LogFactory.getLog(UpdateGatherItem.class);
	...
	public ExecuteResult doAdapterService(IMessage message, String processid)
		...
		try {
			try {
				...
				factory.delGatherItemByGid(Integer.parseInt(int_gatherid), con);
				...
				for (int i = 1; i <= Integer.parseInt(attr_count); i++) {
					...
					if (fcount > 0) {
						...
						if (count > 0) {
							...
							return ExecuteResult.fail;
						} else if (attr_val.get("int_materialruleid" + i) == int_materielruleid
								|| attr_val.get("int_materialruleid" + i)
										.equals(int_materielruleid)) {
							message.addServiceException(new ServiceException(
									ServiceExceptionType.UNKNOWN,
									"生产单元中该采集点物料规则号已为主物料规则号!", this.getId(),
									processid, new Date(), null));
							log.fatal("生产单元中该采集点物料规则号已为主物料规则号。");
							continue;
						} else {
