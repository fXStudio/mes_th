...
public class DAO_CoreForSqlserver extends DAO_Core {
	...
	public String getSQL_QueryNextServiceId() {
		return "select isnull(max(nserverid),0)+1 as serviceid from SERVER_STATEMENT";
	}

	public String getSQL_QueryNextProcessid() {
		return "select isnull(max(nprocessid),0)+1 from process_statement";
	}
   
	public String getSQL_QueryNextNameSpaceId() {
		return "select isnull(max(nnamespaceid),0)+1 as nnamespaceid from namespace_statement";
	}
